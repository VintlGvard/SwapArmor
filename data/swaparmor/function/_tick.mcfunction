...
execute as @e[tag=armor.swap,type=interaction] at @s if data entity @s interaction run data remove entity @e[limit=1,sort=nearest,type=armor_stand,tag=armor.swap.placeholder] equipment
execute as @e[tag=armor.swap,type=interaction] at @s if data entity @s interaction run data remove entity @s interaction

execute as @a[predicate=swaparmor:sneaking,nbt=!{SelectedItem:{}}] at @s run data merge entity @e[type=armor_stand,limit=1,sort=nearest,distance=..5,tag=!armor.swap.placeholder] {Glowing:1b}
